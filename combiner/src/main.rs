...
impl FloatingImage {
  ...
  fn set_data(&mut self, data: Vec<u8>) -> Result<(), ImageDataErrors> {
    if data.len() > self.data.capacity() {
      return Err(ImageDataErrors::BuffeTooSmall);
...
fn main() -> Result<(), ImageDataErrors> {
  ...
  let (image_1, image_2) = standarise_size(image_1, image_2);
...
fn standarise_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
